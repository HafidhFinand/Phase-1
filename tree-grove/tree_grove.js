const {FruitTree, AppleTree, MangoTree, PearTree} = require('./fruitTree.js');
...
class TreeGrove {
  ...
  showAges(){
    console.log('Umur pada pohon2an disini:')
    for(let i in this.treeGrove) {
  ...
  matureTrees(){
    let treeMatured = []
    for(let i in this.treeGrove){
      if (this.treeGrove[i]._age >= this.treeGrove[i]._matureAge && this.treeGrove[i]._age < this.treeGrove[i]._die)
      treeMatured.push(this.treeGrove[i]._name)
  ...
  deadTrees(){
    ...
    for(let i in this.treeGrove){
      if (this.treeGrove[i]._age >= this.treeGrove[i]._die)
      treeDead.push(this.treeGrove[i])
